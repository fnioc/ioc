...
//      through the workspace to its TS source and is bundled in. The package has
//      no real runtime deps (@rhombus-toolkit/func is type-only and erases), so
...
const js = await Bun.build({
  ...
  target: "node",
  format: "esm",
  // No external: @fnioc/core must be bundled, and there are no real runtime
  // deps to keep out (the @rhombus-toolkit type-only imports erase).
});
if (!js.success) {
